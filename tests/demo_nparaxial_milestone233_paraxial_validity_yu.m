...
function test_surface_deferral_local()
    ...
    assert(any(validity.event_table.diagnostic_type == ...
        ...
        'Finite-R surface should use the local true-intersection diagnostic.');
    assert(any(contains(validity.event_table.note, ...
        "Vertex-plane scalar Snell comparison")), ...
        'Finite-R surface diagnostic should preserve vertex-plane information.');
    assert(any(contains(validity.event_table.note, ...
        "Local true-intersection spherical diagnostic")), ...
        'Finite-R surface diagnostic should be clearly labeled as local true-intersection.');
end
...
function status = app_smoke_local(rootFolder)
    ...
    updateValidityButton = findall(app.UIFigure, ...
        '-isa', 'matlab.ui.control.Button', ...
        'Text', 'Update Validity Diagnostics');
    assert(~isempty(updateValidityButton), ...
        'App smoke test could not find Update Validity Diagnostics button.');
    call_callback_local(updateValidityButton(1).ButtonPushedFcn, ...
        updateValidityButton(1));
    drawnow limitrate
