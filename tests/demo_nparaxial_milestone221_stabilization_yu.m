...
function status = test_app_dirty_state_smoke_local(rootFolder)
    ...
    exportMenu = findall(app.UIFigure, 'Text', 'Export Ray Table CSV');
    exportMenu = exportMenu(arrayfun( ...
        @(h) isa(h, 'matlab.ui.container.Menu'), exportMenu));
    assert(~isempty(exportMenu), ...
        'App smoke test could not find ray-table export menu item.');
